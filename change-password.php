                ...
                if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 12) {
                    ...
                    if (mysqli_num_rows($result) === 1) {
                        ...
                        if ($passwd == $hashedPassword) {
                            ...
                            $query = mysqli_query($conn, "UPDATE users SET password='{$hashedPassword}', code='', password_last_updated='{$current_date}' WHERE code='{$_GET['reset']}'");
                            if ($query) {
...
<head>
    <title>SecureAuth | Change Password</title>
